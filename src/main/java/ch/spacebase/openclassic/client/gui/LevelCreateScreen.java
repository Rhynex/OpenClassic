...
public final class LevelCreateScreen extends GuiScreen {
	...
	public void onButtonClick(Button button) {
		...
		if((button.getId() == 1 || button.getId() == 2 || button.getId() == 3) && this.widget.getText().trim().length() > 0) {
			...
			OpenClassic.getClient().createLevel(new LevelInfo(this.widget.getText(), null, (short) (128 << button.getId() - 1), (short) 128, (short) (128 << button.getId() - 1)), OpenClassic.getGame().getGenerator(this.getWidget(0, StateButton.class).getState()));

...
public class AddFavoriteScreen extends GuiScreen {
	...
	public void onButtonClick(Button button) {
		if(button.getId() == 0 && !this.name.getText().equals("")) {
			...
			Storage.getFavorites().put(this.name.getText(), this.url.getText());
	...
	public void onKeyPress(char c, int key) {
		...
		this.getWidget(0, Button.class).setActive(this.name.getText().length() > 0 && this.url.getText().length() > 0);
	}

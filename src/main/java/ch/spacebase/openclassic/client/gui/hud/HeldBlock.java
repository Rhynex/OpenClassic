package ch.spacebase.openclassic.client.gui.hud;
	...
	public void render(float delta) {
		...
		if(OpenClassic.getClient().getHUD().isVisible()) {
			if(this.block != null) {
				...
				GeneralUtils.getMinecraft().player.bindTexture();
				GL11.glScalef(1, -1, -1);
				GL11.glTranslatef(0, 0.2F, 0);
				GL11.glRotatef(-120, 0, 0, 1);
				GL11.glScalef(1, 1, 1);
				GL11.glColor4f(brightness, brightness, brightness, 1);
				ModelPart arm = GeneralUtils.getMinecraft().player.getModel().leftArm;
				if(!arm.hasList) {

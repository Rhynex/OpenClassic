...
public class LevelRenderer {
	...
	private void prepareView(float delta, int anaglyphPass) {
		...
		GLU.gluPerspective(fov, GeneralUtils.getMinecraft().width / (float) GeneralUtils.getMinecraft().height, 0.05f, this.fogEnd);
	...
	private void renderSelected(float delta) {
		Intersection selected = GeneralUtils.getMinecraft().selected;
		if(selected != null) {
			if(GeneralUtils.getMinecraft().cracks > 0) {
				GL11.glDisable(GL11.GL_ALPHA_TEST);
				GL11.glBlendFunc(GL11.GL_DST_COLOR, GL11.GL_SRC_COLOR);
				GL11.glPushMatrix();
				GL11.glDepthMask(false);
				BlockType btype = this.level.getBlockTypeAt(selected.x, selected.y, selected.z);
				if(btype == null) {
					btype = VanillaBlock.STONE;
				}

				Model model = btype.getModel(this.level, selected.x, selected.y, selected.z);
				for(int count = 0; count < model.getQuads().size(); count++) {
					RenderHelper.getHelper().drawCracks(model.getQuad(count), selected.x, selected.y, selected.z, 240 + (int) (GeneralUtils.getMinecraft().cracks * 10));
				}
			...
			BlockType block = this.level.getBlockTypeAt(selected.x, selected.y, selected.z);
			if(block != null) {
				BoundingBox bb = block.getModel(this.level, selected.x, selected.y, selected.z).getSelectionBox(selected.x, selected.y, selected.z);
				if(bb != null) {
			...
			if(selected.hasEntity) {
				selected.entity.renderHoverOver(delta);
			}
		}
	}
	
	private void renderRain(float delta, Player player) {
		if(GeneralUtils.getMinecraft().raining) {
			GL11.glDisable(GL11.GL_CULL_FACE);
			Textures.RAIN.bind();
			for(int cx = player.getPosition().getBlockX() - 5; cx <= player.getPosition().getBlockX() + 5; cx++) {
				for(int cz = player.getPosition().getBlockZ() - 5; cz <= player.getPosition().getBlockZ() + 5; cz++) {
					...
					if(minRY != maxRY) {
						float downfall = (((GeneralUtils.getMinecraft().rainTicks + cx * 3121 + cz * 418711) % 32) + delta) / 32F;

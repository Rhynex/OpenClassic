...
public class MCSharpLevelFormat {
	...
	public static Level load(String file, Class<? extends ClassicLevel> type) throws LevelLoadException {
		try {
			...
			if (magic != 1874) {
				...
				return MCForgeLevelFormat.load(file, type);
			}
			...
			for (int z = 0; z < depth; z++) {
				for (int y = 0; y < height; y++) {
					...
					for (int x = 0; x < width; x++) {
						blocks[ClientLevel.coordsToBlockIndex(x, y, z, width, height, depth)] = translateBlock(row[x]);
					}
				}
			}

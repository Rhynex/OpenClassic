...
public final class LevelGenerator extends Generator {
	...
	public void generate(ch.spacebase.openclassic.api.level.Level level, byte data[]) {
		...
		for (x = 0; x < var46; ++x) {
			...
			for (z = 0; z < var48; ++z) {
				...
				if (((var28 = data[((var26 + 1) * this.depth + z) * this.width + x] & 255) == VanillaBlock.WATER.getId() || var28 == VanillaBlock.STATIONARY_WATER.getId()) && var26 <= var51 / 2 - 1 && var57) {
					data[var27] = VanillaBlock.GRAVEL.getId();
				}
			...
			for (z = 0; z < 10; ++z) {
				...
				for (var26 = 0; var26 < 5; ++var26) {
					...
					if ((var54 < 2 || this.rand.nextInt(4) == 0) && var23 >= 0 && var25 >= 0 && var23 < this.width && var25 < this.depth) {
						...
						if ((data[(var27 * this.depth + var25) * this.width + var23] & 255) == 0) {
							var63 = (var27 * this.depth + var25) * this.width + var23;
							if ((data[((var27 - 1) * this.depth + var25) * this.width + var23] & 255) == VanillaBlock.GRASS.getId()) {
								if (var54 == 0) {
									data[var63] = VanillaBlock.DANDELION.getId();
								} else if (var54 == 1) {
									data[var63] = VanillaBlock.ROSE.getId();
								}
							}
						}
					}
		...
		for (var54 = 0; var54 < var51; ++var54) {
			...
			for (var25 = 0; var25 < 20; ++var25) {
				...
				for (var63 = 0; var63 < 5; ++var63) {
					...
					if ((var24 < 2 || this.rand.nextInt(4) == 0) && var26 >= 0 && var28 >= 0 && var27 >= 1 && var26 < this.width && var28 < this.depth && var27 < var42[var26 + var28 * var46] - 1 && (data[(var27 * this.depth + var28) * this.width + var26] & 255) == 0) {
						int var62 = (var27 * this.depth + var28) * this.width + var26;
						if ((data[((var27 - 1) * this.depth + var28) * this.width + var26] & 255) == VanillaBlock.STONE.getId()) {
							if (var24 == 0) {
								data[var62] = VanillaBlock.BROWN_MUSHROOM.getId();
							} else if (var24 == 1) {
								data[var62] = VanillaBlock.RED_MUSHROOM.getId();
							}
						}
					}
				}
	...
	private long liquify(int var1, int var2, int var3, int var4, int var5, byte data[]) {
		...
		while (var22 > 0) {
			...
			int var14;
			int var15;
			for (var15 = var14 = var2 & var10; var14 > 0 && data[var2 - 1] == 0; --var2) {
				--var14;
			}

			while (var15 < this.width && data[var2 + var15 - var14] == 0) {
				++var15;
			}
			...
			for (; var14 < var15; ++var14) {
				...
				boolean var19;
				if (var3 > 0) {
					if ((var19 = data[var2 - this.width] == 0) && !var23) {
				...
				if (var3 < this.depth - 1) {
					if ((var19 = data[var2 + this.width] == 0) && !var24) {
						if (var22 == this.h.length) {
							var21.add(this.h);
							this.h = new int[1048576];
							var22 = 0;
				...
				if (var13 > 0) {
					...
					if ((var20 == VanillaBlock.LAVA.getId() || var20 == VanillaBlock.STATIONARY_LAVA.getId()) && (var25 == VanillaBlock.WATER.getId() || var25 == VanillaBlock.STATIONARY_WATER.getId())) {
						data[var2 - var1] = VanillaBlock.STONE.getId();
					}
